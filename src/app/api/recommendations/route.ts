...
import {
  findSimilarArticles,
  findSimilarArticlesKeywordBased,
} from "@/lib/embeddings";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const candidateArticles = articles.filter((article) =>
      candidateIds.includes(article.id),
    );
    ...
    try {
      const recommendations = await findSimilarArticles(
        favoriteArticles,
        candidateArticles,
        topK,
      );
      ...
      if (recommendations.length > 0) {
        ...
        return NextResponse.json(
          { recommendations, method: "semantic" },
          { status: 200 },
        );
      }
    } catch (error) {
      console.warn(
        "Semantic search failed, falling back to keyword-based:",
        error,
      );
    }

    // Fallback to keyword-based recommendations
    const recommendations = findSimilarArticlesKeywordBased(
      favoriteArticles,
      candidateArticles,
      topK,
    );
    console.log(
      `Found ${recommendations.length} keyword-based recommendations`,
    );

    return NextResponse.json(
      { recommendations, method: "keyword" },
      { status: 200 },
    );
  } catch (error) {
